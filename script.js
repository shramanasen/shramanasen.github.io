...
// **Disable the Analyze Button Until OpenCV is Loaded**
analyzeBtn.disabled = true;

cv.onRuntimeInitialized = () => {
    console.log("✅ OpenCV.js Loaded!");
    opencvLoaded = true;
    analyzeBtn.disabled = false;  // Enable button when OpenCV is ready
};
...
async function analyzeFaces() {
    if (!opencvLoaded) {
        console.warn("⚠ OpenCV is not loaded yet. Please wait.");
        return;
    }
...
analyzeBtn.addEventListener("click", function() {
    if (!opencvLoaded) {
        console.warn("⚠ OpenCV is still loading. Please wait.");
        return;
    }
    if (!img.src) {
        console.warn("⚠ No image uploaded. Please select an image.");
        return;
    }
    analyzeBrightSpots();
    analyzeFaces();
